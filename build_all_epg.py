...
def fetch_root(url):
    req = urllib.request.Request(
        url,
        headers={"User-Agent": "SenoEPG-ALL/1.1 (+GitHub Actions)"}
    )
...
# NOVA BH
# Izvor:
# BA_Nova.BH.HD.(BIH).ba
#
# Potvrđeni ID koji radi u Smart IPTV:
# 108.ba
...
if source_channel is not None:
    ...
    print(
        f"Nova BH alias added: "
        f"{nova_alias} "
        f"({len(nova_programmes)} programmes)"
    )

else:

    print(
        "WARNING: Nova BH source channel not found"
    )
